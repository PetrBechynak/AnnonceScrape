...
public class AnnonceParser implements IParser {
    ...
    public List<Contact> parseTab(String url) throws Exception {
        ...
        for (Integer i = 1; i < 20; i++) {
            try {
                ...
                String stringDate = detail.select("#content > div.pane.detail-header > div > div.date").text();
                if (stringDate != null && !stringDate.equals("")) {
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d.M.yyyy");
                    contact.setDate(Date.valueOf(LocalDate.parse(stringDate.replaceAll("Datum: ", ""), formatter)));
                }
    ...
    @Override
    public void fillList(AnnonceSettings annonceSettings) throws Exception {
